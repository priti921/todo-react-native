...
const TodoItem = ({children, handleDelete}) => {
  ...
  return (
    <View className='bg-black p-4 rounded-md shadow-2xl my-2 flex-row align-center items-center'>
      <Checkbox value={isChecked} onValueChange={() => setIsChecked(!isChecked)}  className="p-2 mr-5" color={isChecked? 'black': 'white'}/>
      ...
      {isChecked? (<Pressable className='ml-auto'>
        <MaterialIcons name="delete" size={24} color="white" onPress={handleDelete}/>
      </Pressable>) : null}
